...
static async _onRollDamageTest(event){
  ...
  //let itemId = actor.items.get($(event.currentTarget).closest(".item").data("itemId")).
  let subType = "Thrown";
  let item = actor.items.get(itemId);
      ...
      if (usage) {
        if (config.type != "HTD") {
          ...
          if (config.type != "Damage") {
            config.multiShot =  usage.multiShot;
          }    
        }
        config.bonusDice = Number(usage.bonusDice);
        config.boostMP = Math.max(Number(usage.boostMP),0);
        config.cover = Math.max(Number(usage.cover),0);
      }
    }
...
static async dummyVictim(victim, target){
  ...
  let confirmation = await ROLDialog.confirm({
    window: { title: game.i18n.localize('ROL.target')} ,
...
static async RollDialog (options) {
    ...
    const html = await foundry.applications.handlebars.renderTemplate(options.dialogTemplate,data);
      ...
      let formData = null
      const dlg = await ROLDialog.input({
        window: { title: options.winTitle },
        content: html,
        ok:{
  ...
  switch(presetType){
    ...
    case "resolve":
      ...
      if (targetMsg.flags.config.originGM) {
        ...
        const availableGM2 = game.users.find(d => d.active && d.isGM)?.id
        if (availableGM2) {
          game.socket.emit('system.rol', {
            type: 'damagechat',
            to: availableGM2,
